...
public class LearningParameterComponent extends Component {
    ...
    private final SpinnerValueFactory.IntegerSpinnerValueFactory mazeSpinnerRows = new SpinnerValueFactory.IntegerSpinnerValueFactory(2, 16,
            initialRows);
    private final SpinnerValueFactory.IntegerSpinnerValueFactory mazeSpinnerColumns = new SpinnerValueFactory.IntegerSpinnerValueFactory(2,
            16, initialColumns);
    private final SpinnerValueFactory.IntegerSpinnerValueFactory mazeSpinnerEpisodes = new SpinnerValueFactory.IntegerSpinnerValueFactory(1,
            MAX_EPISODES, initialEpisodes);
    ...
    private void resetConfig() {
        config = new TrainingConfig(mazeSpinnerEpisodes.getValue(), mazeSpinnerRows.getValue(), mazeSpinnerColumns.getValue(),
                gamma.getValue(), epsilon.getValue(), alpha.getValue());
    ...
    public Pane build() {
        ...
        hboxSliders.getChildren()
                .addAll(hboxEp, hboxG, hboxA);

        HBox hboxsp = buildSpinners();
        flow.getChildren()
                .addAll(hboxSliders, hboxsp);
        return flow;
    }
    ...
    private HBox buildSlider(Slider slider, String labelTitle, double max) {
        ...
        final Label title = new Label(labelTitle);
    ...
    private HBox buildSpinners() {
        ...
        rowSpinner.valueProperty()
                .addListener((obs, oldValue, newValue) -> {
                    resetConfig();
                });
        columnSpinner.valueProperty()
                .addListener((obs, oldValue, newValue) -> {
                    resetConfig();
                });
        episodeSpinner.valueProperty()
                .addListener((obs, oldValue, newValue) -> {
        ...
        hbox.getChildren()
                .addAll(labelRows, rowSpinner, labelCols, columnSpinner, labelEpisodes, episodeSpinner);
        return hbox;
    }
