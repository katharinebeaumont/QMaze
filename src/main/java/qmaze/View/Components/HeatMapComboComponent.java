...
public class HeatMapComboComponent extends Component {
    ...
    public Pane build() {
        ...
        HBox hbox = new HBox();
        ObservableList<String> options = FXCollections.observableArrayList("Green", "Yellow", "Pink", "Blue", "Cyan", "Red", "None");
        heatMap = new ComboBox<>(options);
        heatMap.setPromptText("Heat map colour");

        heatMap.valueProperty()
                .addListener((ov, t, t1) -> controller.heatMapReset(t1));
        hbox.getChildren()
                .add(heatMap);
        return hbox;
    }
