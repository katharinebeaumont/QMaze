...
public class Maze {
    ...
    private Room getRoom(Coordinates state) {
        ...
        return rooms.stream()
                .filter(r -> r.equals(roomToFind))
                .findFirst()
                .orElseThrow(NotRoomFoundException::new);
    }
    ...
    public List<Coordinates> getAdjoiningStates(Coordinates state) {
        ...
        rooms.stream()
                .filter((otherRoom) -> (otherRoom.isOpen() && otherRoom.adjoins(r)))
                .forEachOrdered((otherRoom) -> adjoiningRooms.add(new Coordinates(otherRoom.getX(), otherRoom.getY())));
